      ...
      <div className="flex min-w-0 flex-1 flex-col justify-between gap-2">
        ...
        {!isPast && onCancel && (
          <button
            ...
            disabled={cancelling}
            className="self-start rounded-lg border border-destructive/40 px-3 py-1 text-xs font-medium text-destructive transition hover:bg-destructive/10 disabled:cursor-not-allowed disabled:opacity-50"
          >
        ...
        <>
          ...
          {past.length > 0 && (
            <section className={cn(upcoming.length > 0 && 'mt-8')}>
              ...
              <div className="space-y-3">
                {past.map((r) => (
                  <BookingCard key={r.id} reservation={r} isPast />
                ))}
              </div>
            </section>
          )}
